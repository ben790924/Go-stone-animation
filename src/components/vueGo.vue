...
<script>
import "../assets/wgo.js";
import "../assets/wgo.player.min.js";
// import '../../wgo.js-master/wgo/wgo.js'
// import '../../wgo.js-master/wgo/kifu.js'
// import '../../wgo.js-master/wgo/basicplayer.js'
// import '../../wgo.js-master/wgo/basicplayer.commentbox.js'
// import '../../wgo.js-master/wgo/basicplayer.component.js'
// import '../../wgo.js-master/wgo/basicplayer.infobox.js'
// import '../../wgo.js-master/wgo/player.js'
// import '../../wgo.js-master/wgo/player.editable.js'
// import '../../wgo.js-master/wgo/player.fullscreen.js'
// import '../../wgo.js-master/wgo/player.permalink.js'
// import '../../wgo.js-master/wgo/scoremode.js'
// import '../../wgo.js-master/wgo/sgfparser.js'
    ...
    loadBasicPlayer() {
      ...
      const player = new WGo.BasicPlayer(elem, {
        ...
        update: a => {
          ...
          //提子陣列
          var removeLength = a.change.remove.length;
          var removeAry = a.change.remove;
          //提子陣列 end

          if (a.change.add[0]) {
            this.stoneColor.push(a.change.add[0]); //取得所以旗子的座標跟顏色
          }

          //canvasDom
          var canvasDom = document.getElementById("player");
          var canvasDomBoard = canvasDom.querySelector(
            ".wgo-player-center > .wgo-player-board"
          );
          //canvasDom end

          //stoneCreation
          let stoneCreation = document.createElement("div");
          stoneCreation.classList.add("stoneDestination");
          stoneCreation.setAttribute(
            "style",
            "background-color: rgb(211, 211, 211); width: 26px; height: 26px; margin: 0; position: absolute; opacity: 1; border-radius: 50%; box-shadow: 1px 5px 6px #999;"
          );
          //stoneDom end

          var canvasDomRect = canvasDomBoard.getBoundingClientRect();
          if (removeLength) {
            //從 add 裡面的顏色變數加入到 remove 裡面
            for (let i = 0; i < this.stoneColor.length; i++) {
              for (let j = 0; j < removeAry.length; j++) {
                if (
                  this.stoneColor[i].x === removeAry[j].x &&
                  this.stoneColor[i].y === removeAry[j].y
                ) {
                  removeAry[j].c = this.stoneColor[i].c;
                }
              }
            }

            this.removedstone.push(removeAry);
            console.log("removeAry", removeAry); // 沒有顏色可以取 所有被吃掉的子的座標位置

            this.removedstone.forEach(stone => {
              var that = this;
              stone.forEach(innerStone => {
                // console.log(innerStone) （x y c）
                // xx,yy 是某格的座標乘以位置
                let xx, yy;
                xx =
                  canvasDomRect.left +
                  a.target.board.fieldWidth * innerStone.x +
                  5;
                yy =
                  canvasDomRect.top +
                  a.target.board.fieldHeight * innerStone.y +
                  10;

                this.restoreInnerStone.push({ xx: xx, yy: yy });

                //改變提子顏色
                if (innerStone.c === -1) {
                  stoneCreation.style.backgroundColor = "white";
                  stoneCreation.classList.add("whiteStone");
                } else {
                  stoneCreation.style.backgroundColor = "black";
                  stoneCreation.classList.add("blackStone");
                }
              });
            });
            // console.log('stonecreation',stoneCreation) 取得stone <div>

            let tl = new TimelineMax();
            var multiStone;
            for (let i = 0; i < this.restoreInnerStone.length; i++) {
              multiStone = stoneCreation.cloneNode();
              // console.log('restoreInnerStone',this.restoreInnerStone) return xx:523 yy:443
              multiStone.style.left = this.restoreInnerStone[i].xx + "px";
              multiStone.style.top = this.restoreInnerStone[i].yy + "px";
              document.body.appendChild(multiStone);
            }
            // console.log('multiStone',multiStone) /* 被clone的 .stoneDestination */

            // let stoneNode = document.childNodes[1].childNodes[2].childNodes
            // console.log(stoneNode) 陣列內第六個以後才是想要的div

            let stoneEle = document.getElementsByClassName('stoneDestination') /* 陣列可操作裡面的 classList */
            
            if(multiStone.style.backgroundColor === 'white'){

              tl.staggerTo(".whiteStone", 1, {
                left: () => {
                  return Math.floor((Math.random() + 1.6) * 100);
                },
                top: () => {
                  return Math.floor((Math.random() + 1.6) * 100);
                }
              }); /* 多顆出去會殺不掉第一個之後的classlist */
              for(let i =0 ; i<stoneEle.length;i++){
                console.log('stoneEle i',stoneEle[i])
                stoneEle[i].classList.remove('whiteStone')
              }
              
            } else if (multiStone.style.backgroundColor === 'black') {

              tl.staggerTo(".blackStone", 1, {
                left: () => {
              ...
              for(let i =0 ; i<stoneEle.length;i++){
                ...
                stoneEle[i].classList.remove('blackStone')
              }
              
            }
            
            this.removedstone = [];
            this.restoreInnerStone = [];

            // console.log(document.body.childNodes[7]) //可抓到提子的div

            //將stoneDestinaiton append 到圈圈內
          }
        }
      });
...
/* Available icons: */
.wgo-icon-fast-forward:before {
  content: "\e817";
}

.wgo-icon-to-end:before {
  content: "\e81a";
}

.wgo-icon-play:before {
  content: "\e80c";
}

.wgo-icon-check:before {
  content: "\e813";
}

.wgo-icon-menu:before {
  content: "\e81b";
}

.wgo-icon-comment:before {
  content: "\e800";
}

.wgo-icon-help-circled:before {
  content: "\e80f";
}

.wgo-icon-check-empty:before {
  content: "\e814";
}

.wgo-icon-circle-empty:before {
  content: "\e816";
}

.wgo-icon-circle:before {
  content: "\e815";
}

.wgo-icon-info:before {
  content: "\e801";
}
...
.wgo-player-left-wrapper,
.wgo-player-right-wrapper {
  height: 100%;
  position: relative;
}
...
.wgo-player-left .wgo-infobox,
.wgo-player-right .wgo-infobox {
  overflow: hidden;
  position: absolute;
  top: 0;
  right: 0;
...
.wgo-player-right .wgo-player-wrapper,
.wgo-player-left .wgo-player-wrapper {
  height: 85px;
  border-bottom: 0;
}
...
.wgo-player-top .wgo-player-info,
.wgo-player-bottom .wgo-player-info {
  position: absolute;
  width: 40%;
  right: 0;
  top: 4px;
...
.wgo-player-top .wgo-player-wrapper,
.wgo-player-bottom .wgo-player-wrapper {
  height: 40px;
  display: inline-block;
  width: 50%;
  margin: 0;
...
.wgo-player-top .wgo-infobox .wgo-box-title,
.wgo-player-bottom .wgo-infobox .wgo-box-title {
  position: absolute;
  right: 40%;
  left: 0;
  margin: 0 5px;
...
.wgo-small .wgo-player-top .wgo-player-info,
.wgo-small .wgo-player-bottom .wgo-player-info,
.wgo-xsmall .wgo-player-top .wgo-player-info,
.wgo-xsmall .wgo-player-bottom .wgo-player-info {
  width: 30%;
}

.wgo-small .wgo-player-top .wgo-infobox .wgo-box-title,
.wgo-small .wgo-player-bottom .wgo-infobox .wgo-box-title,
.wgo-xsmall .wgo-player-top .wgo-infobox .wgo-box-title,
.wgo-xsmall .wgo-player-bottom .wgo-infobox .wgo-box-title {
  right: 30%;
}
...
.wgo-help {
  background-color: rgba(236, 226, 216, 0.9);
  padding: 1px 7px;
  margin-bottom: 5px;
}
...
.wgo-player-right .wgo-commentbox,
.wgo-player-left .wgo-commentbox {
  position: absolute;
  bottom: 0;
  right: 0;
  left: 10px;
...
.wgo-player-top .wgo-commentbox .wgo-box-title,
.wgo-player-bottom .wgo-commentbox .wgo-box-title {
...
.wgo-player-top .wgo-comments-wrapper,
...
.wgo-commentbox.wgo-gameinfo .wgo-info-item:last-child .wgo-info-label,
.wgo-commentbox.wgo-gameinfo .wgo-info-item:last-child .wgo-info-value {
  border-bottom: 0;
}
...
.wgo-button[disabled]::before,
input[type="text"].wgo-player-mn-value[disabled] {
